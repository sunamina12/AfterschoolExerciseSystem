            ...
            <div class="container">
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <th style="text-align:center;">错题数量</th>
                ...
                <div>注：错题数量为你所做的课后作业中做错的且仍未在错题练习中做对的题目数量</div>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="window.location.href=('<%=basePath%>wrong/choice')"/>确认
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
<div class="modal fade" id="startBModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"/>取消
                <button type="button" class="btn btn-primary" onclick="window.location.href=('<%=basePath%>wrong/blank')"/>确认
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
...
<script type="text/javascript">
    function ifstart() {
        var chonum = $("#choWngNum").html();
        var blknum = $("#blkWngNum").html();
        if($.trim(chonum)==0){
            $("#btn_typeC").attr({"disabled":"disabled"});
        }
        if($.trim(blknum)==0){
            $("#btn_typeB").attr({"disabled":"disabled"});
        }
    }
</script>
